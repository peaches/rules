/*
 * Taken from https://github.com/dmajda/pegjs/blob/c13cc882626a245a7a5a97af036f95e40e50a2c7/examples/javascript.pegjs
 *
 * This is a very stripped down version to make a "custom" grammar. It's very much like
 * JavaScript, very slimmed down. Bit operation is not supported, same with advanced
 * operators like +=, %=, or regular expression. Blocks are replaced with "then" and "end"
 *
 * if condition then
 *   // do something
 * end
...
CallExpression
  = first:(
      calleee:MemberExpression __ args:Arguments {
        return { type: "CallExpression", "callee": calleee, arguments: args };
      }
    )
    rest:(
        __ args:Arguments {
...
RelationalExpressionNoIn
  = first:AdditiveExpression
    rest:(__ RelationalOperatorNoIn __ AdditiveExpression)*
    { return buildBinaryExpression(first, rest); }
...
LogicalANDExpressionNoIn
  = first:EqualityExpressionNoIn
    rest:(__ LogicalANDOperator __ EqualityExpressionNoIn)*
    { return buildBinaryExpression(first, rest); }
...
Block
  = ThenToken __ body:(StatementList __)? EndToken {
      return {
        type: "BlockStatement",
...
ElseBlock
  = ThenToken __ body:(StatementList __)? ElseToken {
      return {
        type: "BlockStatement",
        body: optionalList(extractOptional(body, 0))
      };
...
IfStatement
  = IfToken __ test:Expression __
    consequent:StatementElseBlock __
    alternate:Statement
    {
      return {
        type:       "IfStatement",
